...
export function authConfig() {
  const parsed=authConfigSchema.safeParse({accessCode:process.env.ACCESS_CODE,secret:process.env.SESSION_SECRET});
...
export function sameOrigin(request: Request) {
  ...
  return isSameOrigin(request.headers.get("origin"),request.headers.get("host"),new URL(request.url).protocol,request.headers.get("sec-fetch-site"));
}
