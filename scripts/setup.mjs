...
try {
  process.stdout.write(
    "Choose an access code (12–128 letters, numbers, underscores or hyphens; hidden): ",
  );
  const code = await rl.question("");
  process.stdout.write("\n");
  if (!/^[A-Za-z0-9_-]{12,128}$/.test(code))
    throw new Error("Use 12–128 letters, numbers, underscores or hyphens.");
  process.stdout.write("Confirm access code (hidden): ");
  ...
  console.error(
    error.code === "EEXIST"
      ? "Configuration already exists. It was not read or changed."
      : error.message,
  );
  process.exitCode = 1;
} finally {
