...
import { authConfigSchema, createLoginLimiter, createSession, equalCode, isSameOrigin, loginSchema, SESSION_SECONDS, sessionCookieOptions, verifySession } from "../lib/auth/core.ts";
...
test("origin validation handles loopback without trusting cross-site requests",()=>{
  assert.equal(isSameOrigin("http://127.0.0.1:3000","127.0.0.1:3000","http:","same-origin"),true);
  ...
  assert.equal(isSameOrigin("http://localhost:3000","localhost:3000","http:","cross-site"),false);
...
test("tampered payloads, signatures and wrong secrets fail closed",()=>{
  const token=createSession(secret,now);const [payload,signature]=token.split(".");
  ...
  for(const malformed of [undefined,"","x.y","..","x".repeat(2000),token+".extra"]){assert.equal(verifySession(malformed,secret,now),false);}
});
test("signed malformed payloads and extended lifetimes fail validation",()=>{
  for(const data of [{version:2,issued:now/1000,expires:now/1000+SESSION_SECONDS,nonce:"a".repeat(32)},{version:1,issued:now/1000,expires:now/1000+SESSION_SECONDS+1,nonce:"a".repeat(32)},{version:1,issued:0,expires:9999999999,nonce:"bad"}]){
    const p=Buffer.from(JSON.stringify(data)).toString("base64url");const sig=createHmac("sha256",secret).update(p).digest("base64url");assert.equal(verifySession(`${p}.${sig}`,secret,now),false);
  }
});
test("cookie security attributes and configuration are enforced",()=>{
  assert.deepEqual(sessionCookieOptions(true),{httpOnly:true,sameSite:"lax",secure:true,path:"/",maxAge:2592000});
...
test("bounded login limiter resets after its window",()=>{
  const allow=createLoginLimiter(2,1000);assert.equal(allow(1000),true);assert.equal(allow(1001),true);assert.equal(allow(1002),false);assert.equal(allow(2000),true);
});
