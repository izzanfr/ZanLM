...
| T3 Gemini detection and text export | In progress, waiting for the model comparison | `68ca79b`            | Prompt, client, policy, cache, `test:detect`, box refinement and watermark removal done. Model and resolution still unmeasured. Export not started. |
...
- **Box refinement is measured on 36 cached pairs only (2026-09-23).** Shrink-only, with the inside-the-box background estimate for card edges: IoU 0.619 to 0.671 overall; slide 1 0.887 to 0.884, slide 4 0.531 to 0.581, slide 12 0.617 to 0.675, slide 14 0.838 to 0.869. Letting boxes grow scored 0.693 overall but broke slide 1 (0.781); the owner chose shrink-only. **Bias correction is deliberately not applied** (T3 plan, 7.8): measure the shift, the y slope and the width ratio per model and per resolution after the full comparison, and only add it if they agree.
- **The watermark fill is good on 12 of 13 sample slides, not proven beyond them (2026-09-23).** 11 filled from the mirrored other half, 1 from a nearby patch, 1 (slide 6) from the harmonic fill, which still leaves a soft patch. Slide 9 keeps a faint trace where the mirrored frame corner does not line up to the pixel. Only the sample deck has been seen, so the UI option defaults to off and T4 will run the same mask through LaMa (`docs/plan.md`).
...
1. **Set the machine up** (see "Moving to another computer" below): clone, `npm ci`, write `.env.local` with `npm run setup`, put `GEMINI_API_KEY` in it by hand, and unpack the samples-and-cache archive, which is git-ignored and therefore not on GitHub.
2. **Morning WIB, when Google is quieter: rerun the full comparison** with the guards:
   `npm run test:detect -- --model gemini-3.5-flash-lite --model gemini-3.1-flash-lite --model gemini-3.8-flash --media-resolution default --media-resolution high`
   Cached answers cost nothing, so only what is missing is called. Report the metrics and stop for the owner to choose model and resolution.
3. **Then measure box bias per model and per resolution** on those results and decide whether a correction goes into the code (conditions in the T3 plan, 7.8).
4. **Then the export commits** (T3 plan, section 11, from commit 5), with the refined boxes, the watermark option (off by default) and the resume behaviour in section 8. Nothing of the PPTX export exists yet.
5. Owner checks reduced motion with the checklist above.
...
2. `npm run setup` to write `.env.local` (access code and session secret), then add `GEMINI_API_KEY=...` to that file by hand. It is never read or printed by the agent.
