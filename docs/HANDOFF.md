...
| T4 Worker and text inpainting       | Planned, waiting for approval   | `docs/T4-plan.md`: text mask first (no Python), then the worker. Nothing installed. |                                                                                                                                                                            |
...
| 2026-09-23 | Text box height is computed from the font size, lines and real metrics; the detected box gives position and width, anchored at the top        | The one bias every model and resolution agrees on is that boxes are 7 to 13% too short, and a short box clips its text in PowerPoint                                                                                                    | Taking the detected height; correcting the height by a factor; leaving it to the Review page in T7 |
| 2026-09-23 | A fill candidate is refused when its grain is far from the background's, and the smooth fill is preferred over one that is                    | Matching detail alone pasted stars into corners that had none (slides 5, 7, 13, 14)                                                                                                                                                     | Keeping the detail-only match; always using the smooth fill                                        |
...
The T3 plan (7.8) allows a correction only when direction and magnitude agree across the runs. They do not. The width ratio runs from 0.993 to 1.127, so the earlier "15% too wide" is one model's habit, not the family's; for the chosen combination width is already right (1.012). The vertical shift keeps its sign but varies almost threefold, and its spread per pair (+-0.66 to +-1.27) is as large as the bias itself. The y slope is 0.996 for the chosen combination, so the earlier 0.974 ("boxes drift higher lower down the slide") is not reproduced either. **No bias correction goes into the code.** The one consistent number is height: boxes are 7 to 13% too short in every run, which shrink-only refinement cannot help. The owner's decision of 2026-09-23 handles it in the exporter instead of correcting the box: the text box height is computed from the font size, the line count and the real font metrics, the detected box gives only position and width, and the box is anchored at its top edge (T3 plan, 7.4, with a test that text is never clipped).

## Known issues

- **T4 dependency report (2026-09-23, nothing installed).** LaMa's code and the `big-lama` weights are both Apache-2.0, with no non-commercial clause found in the repository, the README or the model cards, so no licence note is needed in AGENTS.md. MI-GAN, which looks permissive because its repository is MIT, is the restricted one: its weights come from a Co-Mod-GAN teacher under the NVIDIA non-commercial licence. Sizes checked on the real indexes: `onnxruntime` 13.6 MB with `lama.onnx` 197.9 MB, against `torch` (CPU) 118 MB with `big-lama.zip` 363.8 MB; `opencv-python-headless` 41.8 MB. The recommendation is the ONNX path on CPU, inpainting a crop around the mask rather than the whole slide. Full table in `docs/T4-plan.md`, 4.2. **Waiting for the owner's approval before anything is installed.**

- **Both exported decks are temporary, and T4 is what fixes them (owner, 2026-09-23, after opening the first export).** The original text is still in the background picture, so without cover patches every line reads twice. With patches it reads once, but a patch is a flat rectangle on a textured slide, and it is sized from the detected box, which is 7 to 13% too short: on slides 4 and 12 the old text still showed under the new one ("n" of a heading, "(Abu-abu)", "konsentrasi tinggi", "Eksfoliasi lembut", and most of slide 4's paragraphs). **Cover patches are therefore off by default**, and the option only becomes worth using when the patch is sized from a mask of the original text pixels rather than from the detected box. That mask is the same one T4 hands to LaMa, so it is built once and used twice.
- **Text size now follows the ink on the slide (measured 2026-09-23, 0 requests).** `npm run test:sizes` compares the chosen line height with the one in the picture: the baseline pitch for a block of several lines, the ink height against the same face's cap-plus-descender for a single line. Over the whole sample deck, 87 blocks with a cached detection:

| Rule                                            | Mean     | Median   | Min      | Max      | Outside 0.9 to 1.1 |
| ----------------------------------------------- | -------- | -------- | -------- | -------- | ------------------ |
| Height from the detected box (before)           | 0.87     | 0.78     | 0.37     | 4.93     | 69 of 87           |
| Height from the ink, width from the box         | 0.82     | 0.80     | 0.37     | 1.21     | 58 of 87           |
| **Height from the ink, box free to grow (now)** | **0.98** | **0.99** | **0.50** | **1.21** | **4 of 87**        |

The middle row is why the box may now grow sideways: the width limit decided 67 of 87 blocks, because Arial and Georgia are wider per character than the deck's own faces, so obeying the detected width shrank almost everything. Each of the four blocks still outside is named with its reason, never averaged away: slide 3's title at 1.21 is a 5.6 pt line meeting the 6 pt floor; slide 3's heading at 0.50 and slide 4's two paragraphs at 0.53 and 0.83 were **narrowed to fit**, because at the original line height their text would have grown into a neighbouring block. Growing boxes cost 0 overlapping pairs: the layout checks every pair and sends a block back inside its own width when it would collide (before this guard, 2 pairs overlapped). 19 further blocks are not measured at all, because the mask will not vouch for them; they keep the old rule and are flagged `size-from-box`.

- **Two blocks the size rule cannot reach (2026-09-23).** Slide 3's heading and slide 4's second paragraph sit at 0.50 and 0.53 because their original face is far narrower than Arial: at the right line height the text needs more width than the slide gives it. A condensed face for blocks whose ink is much narrower than our measurement would fix it; that belongs with the Review page in T7, not to a blind rule.
- **The export has not been opened in real PowerPoint yet.** LibreOffice renders it, but font size and line wrapping there are not PowerPoint's. `verify-pptx.ps1` (T3 plan, section 9) is still to be written and run; until then every size statement above is a measurement on the picture, not on a rendered slide.
...
- **The watermark option is on by default (owner decision 2026-09-23), and one slide still shows it.** The words are gone on all 13 marked slides (0.00 to 0.04 of the mark's own pattern survives, where 1.00 is the untouched slide), and since the grain guard no slide pastes texture the corner never had. **Slide 8 is the exception**: its mark sits inside the gold ornament, so the fill rebuilds ornament strands that are plausible but not the original ones. What shows there is a slightly different ornament, not a watermark. T4 runs the same mask through LaMa (`docs/plan.md`), which is the fix. Only the sample deck has been seen. Removal costs about 1.5 s per slide.
...
1. **T4 (LaMa) is the next feature, and it is what both exports are waiting for**: the original text has to leave the background picture. **The plan is written: `docs/T4-plan.md`, waiting for the owner's approval.** Nothing is installed. Its first three commits need no Python at all: one text-pixel mask module, used by the inpainting, by the cover patch and by the font size, which is what should close the size spread of 0.40 to 1.47.
