...
- **The chosen text size misses in both directions (measured 2026-09-23, 0 requests).** On the real export path (cached detections, refined boxes, real font metrics) the chosen line height was compared with the line height in the picture: for a block of several lines the baseline pitch from the first ink row to the last, for a single line the ink height against the same face's cap-plus-descender. Over 42 blocks the mean ratio is 0.98 and the median 1.00, so there is no bias to correct, but the spread is wide: 9 blocks are more than 10% too large (worst: slide 12's subtitle at 1.47, slide 4's third card at 1.21) and 11 are more than 10% too small (worst: slide 4's heading at 0.40, slide 9's body at 0.78). The cause is the detected box height, which the size still depends on through the height limit. Nothing is corrected yet: measure first was the owner's instruction, and a fix has to come from the text pixels, like the patch mask.
