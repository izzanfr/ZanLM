...
- **T4, watermark area (owner decision 2026-09-22):** T3 removes the NotebookLM mark with a pixel fill from its surroundings (`docs/T3-plan.md`, section 7.9). On a busy or gradient background that leaves a soft patch. When the worker exists, run the watermark area through LaMa with the same mask, along with the text areas.
- **T3, mixed slides (owner decision 2026-09-22):** a PPTX slide flagged `mixed` in `job.json` (anything other than one full-bleed picture) already carries native text boxes and shapes. T3 should keep those native elements as they are, not detect them again from the extracted picture, and only run detection over the picture itself. The same flag on a PDF page only means the page was rendered; a PDF has no native elements to keep.
