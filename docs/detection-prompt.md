...
This file is the source of truth for the prompt that ZanLM sends to Gemini with every slide image. It was approved by the owner on 2026-09-22 (`docs/T3-plan.md`, section 4.3). **Change it only when the owner asks.**

- The prompt is the text inside the single fenced block below, exactly, without the fence lines.
- `lib/prompts/detection.ts` holds the same text as a constant, and `tests/detection-prompt.test.ts` fails if the two differ by a single character.
- `PROMPT_VERSION` is derived from the SHA-256 of this text, so any change also invalidates cached Gemini answers.
- The JSON response schema is defined in code (`lib/gemini/schema.ts`, Zod) and described in `docs/T3-plan.md`, section 4.2.
