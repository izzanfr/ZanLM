...
- The JSON Schema sent to Gemini (`lib/gemini/schema.ts`) is written out from the same enum and limit constants as the Zod schema, not generated with `z.toJSONSchema()`: the Zod schema carries transforms (color normalisation, the runs check) that do not belong in what Gemini is asked to produce. A unit test keeps the two in step, and checks that only simple JSON Schema keywords are used.
...
Two guards keep a run from hanging on an overloaded model (added 2026-09-22, after a 30-minute run spent entirely on 503s). `--max-attempts <n>` (default 3) caps the real requests per item; when all of them fail the item is marked unavailable and the run moves on. `--budget-minutes <n>` (default 10) caps the whole run; when it is spent the run stops, cancels a request in flight, and reports what was finished. Only combinations whose every item was measured are summarised; the others are reported as not measured, with their request codes. Every response code is counted per combination and per model. The app keeps the plain policy. `npm run gemini:check` lists the models the key can use, and says whether each `GEMINI_MODEL*` variable is set and available, without printing values.
